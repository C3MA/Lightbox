...
void setup()
{
  artnet = new Artnet("192.168.2.60" /* The IP address of the Artnet device */, 
...
void draw()
{
  ...
  for(int i=0; i < NETWORK_SIZE; i++) {
    ...
    sendPWMLightBoxBuffer(magic(r >= 0 ? output[r] : 0), magic(g >= 0 ? output[g] : 0), magic(b >= 0 ? output[b] : 0), i);
    r=-1; 
    g=-1; 
    b=-1;
  }
...
synchronized void sendPWMLightBoxBuffer(int r, int g, int b, int id) {
  bang[id].setColorForeground(color(r,g,b));
  id = id * 4; /* id is now used as index */
  universeBuffer[id++] = (byte) r;
  universeBuffer[id++] = (byte) g;
  universeBuffer[id++] = (byte) b;
  universeBuffer[id++] = (byte) 255; /* needed for the 'Sev-Eimer' */
}

void sendUniverse() {  
  artnet.send(universeBuffer);
